...
public class directoryNavigator {
    ...
    public void setCurFolder(String folderName){ curFolder = new File(PATH + "/" + folderName); }

    public File getResFolder() { return resFolder; }

    public void setResFolder(String newResFolder) {
        String[] withoutHome = newResFolder.split("/"); //Index 2 of the array is where the /Users/username/ stuff ends.
        String folderString = "";
        for(int x = 3; x < withoutHome.length; x++){ folderString = folderString + "/" + withoutHome[x]; }
        try{ if(withoutHome.length > 3) Files.write(readMe.toPath(), (readMeStr.substring(0, readMeStr.indexOf(": ") + 2) + folderString + readMeStr.substring(readMeStr.indexOf("~"))).getBytes());
             else Files.write(readMe.toPath(), (readMeStr.substring(0, readMeStr.indexOf(": ") + 2) + readMeStr.substring(readMeStr.indexOf("~")-1)).getBytes()); }
        catch(IOException e) { System.out.println(e); }
